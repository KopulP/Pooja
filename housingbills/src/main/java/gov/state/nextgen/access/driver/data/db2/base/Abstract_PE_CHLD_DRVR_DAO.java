...
import gov.state.nextgen.access.driver.business.entities.PE_CHLD_DRVR_PrimaryKey;
...
/**
 * Abstract Data Access Object for PE_CHLD_DRVR
 *
 * @author CodeGenerator - Architecture Team
 * Creation Date Thu Jul 26 15:18:54 CDT 2007
 */
public class Abstract_PE_CHLD_DRVR_DAO extends FwAbstractDAO {

	private static final short HISTORY_TYPE = 0;

	private static final String SELECT_SQL = "SELECT APP_NUM,APCCI_STAT_IND,APCCS_STAT_IND,APCHR_STAT_IND,APCID_STAT_IND,APCII_STAT_IND,APCPS_STAT_IND FROM "
			+ getDBSchema() + FwConstants.DOT + "PE_CHLD_DRVR";
	private static final String INSERT_SQL = "INSERT INTO  " + getDBSchema() + FwConstants.DOT
			+ "PE_CHLD_DRVR(APP_NUM,APCCI_STAT_IND,APCCS_STAT_IND,APCHR_STAT_IND,APCID_STAT_IND,APCII_STAT_IND,APCPS_STAT_IND) VALUES(?,?,?,?,?,?,?)";
	private static final String PK_SELECT_SQL = "SELECT 1 FROM " + getDBSchema() + FwConstants.DOT + "PE_CHLD_DRVR WHERE APP_NUM=?";
	private static final String UPDATE_SQL = "UPDATE "
			+ getDBSchema()
			+ FwConstants.DOT
			+ "PE_CHLD_DRVR SET APCCI_STAT_IND=?,APCCS_STAT_IND=?,APCHR_STAT_IND=?,APCID_STAT_IND=?,APCII_STAT_IND=?,APCPS_STAT_IND=? WHERE APP_NUM=?";
	private static final String DELETE_SQL = "DELETE FROM " + getDBSchema() + FwConstants.DOT + "PE_CHLD_DRVR WHERE  APP_NUM=?";
	 ...
	 */
	private void getRowFromResultSet(final PE_CHLD_DRVR_Cargo row, final ResultSet rs) {

		try {
			row.setApp_num(rs.getString("APP_NUM"));
			row.setApcci_stat_ind(rs.getString("APCCI_STAT_IND"));
			row.setApccs_stat_ind(rs.getString("APCCS_STAT_IND"));
			row.setApchr_stat_ind(rs.getString("APCHR_STAT_IND"));
			row.setApcid_stat_ind(rs.getString("APCID_STAT_IND"));
			row.setApcii_stat_ind(rs.getString("APCII_STAT_IND"));
			row.setApcps_stat_ind(rs.getString("APCPS_STAT_IND"));
	...
	public ICargo[] select(final FwDataCriteria[] aCriteria, final FwDataSortOrder[] aSort, final Connection aConn) {
		...
		ResultSet rs = null;
		PE_CHLD_DRVR_Cargo[] rows = null;
		final StringBuilder whereClause = new StringBuilder();
		try {
			conn = aConn;
			if (aCriteria.length == 0) {
				log(ILog.INFO, "Abstract_PE_CHLD_DRVR_DAO.select() - Raising FwException manually on a condition");
			...
			rows = new PE_CHLD_DRVR_Cargo[values.size()];
			values.toArray(rows);
		} catch (final SQLException sqle) {
			final FwException fe = new FwException(sqle);
	 ...
	 * @param aKey associated primary key
	 * @param aConn jdbc database connection
	 * @return associated record on cargo format
	 */
	public ICargo findByPrimaryKey(final IPrimaryKey aKey, final Connection aConn) {
		Connection conn = null;
		PreparedStatement statement = null;
		ResultSet rs = null;
		PE_CHLD_DRVR_Cargo rescargo = null;
		final PE_CHLD_DRVR_PrimaryKey key = (PE_CHLD_DRVR_PrimaryKey) aKey;
		try {
			int count = 1;
			conn = aConn;
			...
			rs = statement.executeQuery();
			if (rs.next()) {
				rescargo = new PE_CHLD_DRVR_Cargo();
			}
		} catch (final SQLException sqle) {
			final FwException fe = new FwException(sqle);
			FwExceptionManager.setSqlca(sqle, fe);
	...
	public boolean insert(final ICargo aCargo, final Connection aConn) {
		final PE_CHLD_DRVR_Cargo cargo = (PE_CHLD_DRVR_Cargo) aCargo;
		Connection conn = null;
		PreparedStatement statement = null;
		try {
			int count = 1;
			...
			statement.setShort(count++, Short.parseShort(cargo.getApcci_stat_ind()));
			statement.setShort(count++, Short.parseShort(cargo.getApccs_stat_ind()));
			statement.setShort(count++, Short.parseShort(cargo.getApchr_stat_ind()));
			statement.setShort(count++, Short.parseShort(cargo.getApcid_stat_ind()));
			statement.setShort(count++, Short.parseShort(cargo.getApcii_stat_ind()));
			statement.setShort(count++, Short.parseShort(cargo.getApcps_stat_ind()));
			final int res = statement.executeUpdate();
			if (res == 0) {
				log(ILog.INFO, "Abstract_PE_CHLD_DRVR_DAO.insert() - Raising FwException manually on a condition");
	...
	public boolean update(final ICargo aCargo, final Connection aConn) {
		...
		try {
			...
			final int res = statement.executeUpdate();
			if (res == 0) {
				log(ILog.INFO, "Abstract_PE_CHLD_DRVR_DAO.update() - Raising FwException manually on a condition");
	...
	public boolean delete(final ICargo aCargo, final Connection aConn) {
		...
		try {
			...
			final int res = statement.executeUpdate();
			if (res == 0) {
				log(ILog.INFO, "Abstract_PE_CHLD_DRVR_DAO.delete() - Raising FwException manually on a condition");
