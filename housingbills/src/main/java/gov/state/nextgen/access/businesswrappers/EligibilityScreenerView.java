...
import gov.state.nextgen.access.model.AmIELigibleQuestions;
...
@Component("ACRES")
@Scope("prototype")
public class EligibilityScreenerView extends CommonResponseBean {

	private static final String PAGE_ID = "ACRES";
	...
	public PageResponse constructPageResponse(FwTransaction txBean)
	{
		...
		Map eligResMap = session.get("EligibilityResults") != null ? (Map) session.get("EligibilityResults"): new HashMap();
		CP_APP_SCREENER_Collection cpAppScreenerCollection = eligResMap.get("CP_APP_SCREENER_Collection") != null ? (CP_APP_SCREENER_Collection)eligResMap.get("CP_APP_SCREENER_Collection") : null;
		
/*		List<AmIELigibleQuestions> aieQuesList = new ArrayList<AmIELigibleQuestions>();
		AmIELigibleQuestions aieQues = new AmIELigibleQuestions();
*/		
		...
		List<AmIEligibleResults> aieResList = new ArrayList<AmIEligibleResults>();
		AmIEligibleResults aieRes = new AmIEligibleResults();
		
		if(cpAppScreenerCollection != null) {
			cargo = (CP_APP_SCREENER_Cargo) cpAppScreenerCollection.get(0);
			aieRes.setIsEligibleForMedicaid(cargo.getScr_elg_rslt_ma());
			aieRes.setIsEligibleForSNAP(cargo.getScr_elg_rslt_snap());
			aieRes.setIsEligibleForTANF(cargo.getScr_elg_rslt_tanf());
			aieRes.setIsEligibleForChildCare(cargo.getScr_elg_rslt_cc());
			aieRes.setIsEligibleForWIC(cargo.getScr_elg_rslt_wic());
		}
...
/*		
	...
	*/	
		aieResList.add(aieRes);
		driverPageResponse.getPageMap().put("AmIEligibleResults", aieResList);
...
*/		
		...
		//beforeCollection.put("CP_APP_SCREENER_Collection",cpAppScreenerCollection);
